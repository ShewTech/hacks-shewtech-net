...
function GetQuestion() {
  return [...document.querySelectorAll('div')]
    ...
    .filter(
      (e) =>
        e &&
        e.hasOwnProperty('data-test') &&
        e['data-test'].includes('challenge')
    )[0];
}

function GetChallenge(e = GetQuestion()) {
  return (
    e.children.props.children[1].props.challenge ??
    GetQuestion().children.props.children[1].props.children[1]?.props
      ?.challenge ??
    GetQuestion().children.props.children[1].props.children.props.challenge
  );
}
    ...
    try {
      ...
      ({
        'challenge challenge-translate': () => {
          if (
            l.challengeGeneratorIdentifier.specificType === 'reverse_translate'
          ) {
            React(
              document.querySelector(
                "textarea[data-test='challenge-translate-input']"
              )
            ).onChange(Value(l.correctSolutions[0])),
              setTimeout(DoubleNext, 200);
            return;
          } else {
            ...
            new Promise((resolve) => {
              ...
              let interval = setInterval(() => {
                ...
                const word = words.find(
                  (word) => word.innerText === correctToken
                );
                ...
                const indexToRemove = words.findIndex(
                  (word) => word.innerText === correctToken
                );
                if (indexToRemove !== -1) {
        ...
        'challenge challenge-select': () => {
          React(
            document.querySelectorAll('div[data-test="challenge-choice-card"')[
              l.correctIndex
            ]
          ).onClick({}),
            DoubleNext();
        ...
        'challenge challenge-dialogue': () => {
          DoubleNext(),
            React(
              document.querySelectorAll('div[data-test="challenge-choice"]')[
                l.correctIndex
              ]
            ).onClick({}),
            DoubleNext();
        ...
        'challenge challenge-name': () => {
          ...
          ).onChange(Value(l.correctSolutions[0].split(' ')[0] + ' ')),
            setTimeout(
              () =>
                React(
                  document.querySelector(
                  ...
                  Value(
                    React(
                      ...
                      l.correctSolutions[0].split(' ')[0] + ' '
                      ? l.correctSolutions[0]
                      : l.correctSolutions[0].split(' ')[1]
                  )
                ),
              100
            ),
        ...
        'challenge challenge-match': () => {
          ...
          e.forEach((e, t) => {
            setTimeout(
              () =>
                e.forEach((e, t) =>
                  setTimeout(() => React(l[c.indexOf(e)]).onClick({}), 250 * t)
                ),
              500 * t
            );
          }),
        ...
        'challenge challenge-completeReverseTranslation': () => {
          ...
          ).onChange(Value(l.displayTokens.reduce((e, t) => e + t.text, ''))),
            DoubleNext();
        },
      })[e['data-test']]();
    } catch {
...
function Next() {
  (
    document.querySelector('button[data-test="player-next"') ??
