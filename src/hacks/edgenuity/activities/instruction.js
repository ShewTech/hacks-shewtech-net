...
RegisterActivity(['Instruction', 'Warm-Up'], () => {
  /**
  ...
  if (API.Frame.isComplete()) {
    ...
    if (API.Frame.StackProgress.length === 0) {
      // try {
      //   API.Video.setTimeViewed(API.Video.totalDuration - 1);
      // } catch (e) {
      //   console.warn('failed to set video time viewed', e);
      // }
    } else {
          ...
          responses
            ...
            .forEach((response) => {
              response.data.json().then((data) => {
                ...
                const select = API.childWindow.$(
                  `[qid="${response.id}"] select`
                );
